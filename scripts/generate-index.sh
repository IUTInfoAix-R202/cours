#!/bin/bash
# Génère _site/index.html, la page d'accueil du site Pages.
#
# Une ligne de tableau par CM. Si le fichier source .md existe :
# liens HTML + PDF. Sinon : cellule unique avec un statut (en cours /
# à venir).
#
# Appelé par :
# - .github/workflows/marp-pages.yml (step Generate index page)
# - scripts/build-all.sh (build local complet)
#
# Préalable : _site/ doit exister (créé par les steps de build HTML).
